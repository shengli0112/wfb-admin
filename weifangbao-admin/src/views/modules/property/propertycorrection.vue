  ...
  <el-card shadow="never" class="aui-card--fill">
    <div class="mod-message propertycorrection}">
      ...
      <el-table v-loading="dataListLoading" :data="dataList" border @selection-change="dataListSelectionChangeHandle"
        ...
        <el-table-column prop="description" label="错误内容" header-align="center" align="center"></el-table-column>
        <el-table-column prop="screenshot" label="图片" header-align="center" align="center">
          <template slot-scope="scope">
            <div class="imgs" style="width: 20%; height: 20%; margin:auto;display: inline-block"
                 v-for="(item,index) in scope.row.screenshot" :key="index"
        ...
        <el-table-column prop="createDate" label="上传时间" header-align="center" align="center"></el-table-column>
...
<script>
    ...
    import AddOrUpdate from './propertycorrection-add-or-update'
    ...
    export default {
        ...
        data() {
            return {
                mixinViewModuleOptions: {
                    getDataListURL: '/property/propertycorrection/page',
                    getDataListIsPage: true,
                    exportURL: '/property/propertycorrection/export',
                    deleteURL: '/property/propertycorrection',
                    deleteIsBatch: true
                },
                total: 0,
                dataForm: {
        ...
        methods: {
            search() {
                this.$http.get('property/propertycorrection/page?propertyName=' + this.dataForm.propertyName + '&status=' + this.dataForm.status).then(({data: res}) => {
                    if (res.code !== 0) {
                        return this.$message.error(res.msg)
                    }
                    res.data.list.forEach(function (item, index) {
                        res.data.list[index].screenshot = JSON.parse(item.screenshot)
                    })
            ...
            getDataList() {
                this.$http.get('property/propertycorrection/page').then(({data: res}) => {
                    if (res.code !== 0) {
                        return this.$message.error(res.msg)
                    }
                    res.data.list.forEach(function (item, index) {
            ...
            changeDetail(row) {
                this.$confirm('处理该反馈, 是否继续?', '提示', {
                    ...
                    this.$http.put('/property/propertycorrection', {
                        "id": row.id,
                        "status": "1"
                    }).then(({data: res}) => {
                        if (res.code !== 0) {
